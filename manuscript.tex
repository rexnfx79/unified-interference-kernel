...
\begin{abstract}
We present a framework for organizing Yukawa couplings through envelope suppression and phase interference. A single kernel form is applied across all fermion sectors, with different sectors corresponding to distinct projection regimes. Three regimes are identified: envelope-dominated (quarks), phase-sensitive (charged leptons), and metric-dominated (neutrinos). The primary finding is that the quark sector exhibits a structural CKM--$m_c$ trade-off characterized by a nondominated Pareto frontier. Secondary findings include successful reproduction of charged lepton masses (60\% survivor rate) and PMNS mixing angles (45\% survivor rate), with all survivor Z-scores below 1.0. Exploratory observations suggest consistent parameter scales across sectors ($\sigma \approx 4$--5, $\alpha \approx 2.5$--3.0) and a correlation between envelope compression and large neutrino mixing angles. These results demonstrate that the three-regime framework provides a coherent phenomenological organization of fermion flavor structure.
\end{abstract}
...
This work presents comprehensive optimization results across all three fermion sectors, demonstrating that the three-regime framework successfully reproduces experimental observables with high precision. We identify consistent parameter scales across sectors, characterize Pareto boundary shapes, and observe that envelope compression correlates with large mixing angles that mimic anarchical mixing patterns.
...
\textbf{Charged Lepton Sector: Partial Success.} The phase-sensitive regime achieves 60\% success rate at 10\% tolerance (Fig.~\ref{fig:success_threshold}), primarily driven by $m_\tau$ predictions. The muon mass shows large scatter, and the claimed ``phase parameter correlation'' is not statistically significant.

\textbf{Neutrino Sector: Qualified Success.} The metric-dominated regime achieves 39\% success rate at 10\% tolerance for mixing angles. However, 50\% of optimizations failed entirely, and the claimed correlation between $g_{\text{env}}$ and mixing angles is weak ($r < 0.1$).
